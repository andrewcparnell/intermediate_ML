...
for (i in 5:5) { # Change loop to show more or fewer images
  img <- train_images[i,,,]
...
encoding_dim <- 256 # Need probably quite a few dimensions here
...
# Compile the model
autoencoder %>% compile(optimizer = keras$optimizers$legacy$Adam(learning_rate = 0.01), 
...
# Compile
autoencoder_conv %>% compile(optimizer = keras$optimizers$legacy$Adam(learning_rate = 0.01), 
                             loss = 'binary_crossentropy')
